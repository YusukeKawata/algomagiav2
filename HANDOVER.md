...
- ✅ **済（続8・PR #8 マージ済み）**：第1幕の旅路を豊かに＝HUDに現在HP/自由意志／旅の野営地（焚き火＝休息＋内省）×5／旅の行商人（補給＋魔石売却）／**看破からの反撃**（`heroCounter`・autoWinnable不変）／**にげる**（覚醒後＆遭遇のみ＝詰み防止不変）／敵ごとの遭遇フレーバー／魔物図鑑（討伐数）／旅路BGM「travel」／記録帳の分類カウンタ。**ロック済み設計は無改変・追加のみ**。code-review(high)で指摘修正（doFlee堅牢化・maps.testのNPC考慮・DRY）も同梱。100テスト/typecheck/build 緑。

**次の候補（上から着手推奨／第1幕の旅路の肉付けは続8で一区切り）**
1. **第2幕本体**＝第三の町（壁の狩人町・ハブ&スポーク・依頼/周回・魔石経済 §8.10）。地中の里[O]の先に接続。精霊魔法(霊脈 §8.12)・並列演算(演算 §8.13)の解禁スポーク。**着手は方式A（小さい複数案を出して ponti が選ぶ）で**。盤の核（心域=横・演算=縦）がここで本格始動する＝第1幕で温存した signature mechanic の payoff。
...
4. **店/装備/道具のバランス**：属性防具の価格・性能カーブ、状態異常薬、終盤装備、ゴールド収支。`equipment.ts`/`items.ts`/`shops.ts`/`stones.ts`。旅の行商人（`road` 店）に装備を足すなら多種別ショップ対応が要る。
5. **磨き**：BGMの音色/進行・戦闘トラック分化／garo_house の壁タイル（今は墓石流用）／**人物アート（立ち絵）**（色トークン代用中・供給されたら差し替え）。
6. **要相談（バランス調整）**：中盤グラインドのさらなる軽減（遭遇率/マップ規模）。`autoWinnable` のレベル前提に関わるので ponti と方針を決める。
