...
**土台＋魔石盤（属性・最小戦闘まで）完成。** `npm run dev` で1画面のコアループが遊べる＝盤に回路を組む→[F]で撃つ→弱点で倍ダメージ→自由意志が尽きる→勝敗。
- `src/core/board.ts`（回路判定＋属性組成→元素・決定論・テスト8件）／`src/core/battle.ts`（自由意志コスト=石数・ダメージ=強さ・弱点×2・自由意志0で敗北・テスト6件）。
- `src/app/BoardScene.ts`（5×4盤＝数字=形/qwer=属性/左ｸﾘｯｸ=置く/右ｸﾘｯｸ=消す、回路点灯・元素色・敵HP/自由意志バー・[F]撃つ/[N]新しい敵）。
- `npm run typecheck` / `npm run test`（15件）/ `npm run build` すべて緑。**他の本実装（タイトル/フィールド/会話/物語/盤の成長）はこれから。** ※`BootScene.ts` は未使用（後でタイトルに転用候補）。
- **戦闘の数値は暫定（手書き・調整可）**：コスト=石数／ダメージ=強さ／弱点×2／敵干渉=自由意志-2。方式Aで詰める。
