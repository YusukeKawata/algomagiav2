...
/** start から到達できる床セル集合（4近傍BFS）。blocked＝床でも通れない座標（NPC等＝ゲーム内 blocked() 相当）。 */
function reachable(map: FieldMap, sx: number, sy: number, blocked = new Set<string>()): Set<string> {
  ...
  while (q.length) {
    ...
    for (const [dx, dy] of [[0, -1], [0, 1], [-1, 0], [1, 0]] as const) {
      ...
      if (blocked.has(k)) continue;                 // NPC が乗ったマスは通れない（ゲーム内 blocked() と整合）
...
/** マップ上の NPC が乗っている座標集合（ゲーム内では通れない＝連結性に影響する）。 */
function npcBlockedSet(map: FieldMap): Set<string> {
  return new Set((map.npcs ?? []).map((n) => `${n.x},${n.y}`));
}
...
describe('マップ: 生成ダンジョンの連結性（孤立して詰まない＝不変条件）', () => {
  ...
  it('野営地（焚き火）の座標は床で、各マップの出口から到達できる', () => {
    for (const id of Object.keys(MAPS)) {
      const m = MAPS[id]!;
      for (const camp of m.camps ?? []) {
        expect(walkable(tileAt(m, camp.x, camp.y)), `${id} の野営地(${camp.x},${camp.y})が床でない`).toBe(true);
        // マップの最初の exit 着地点から野営地へ歩いて行ける（孤立して使えない事故を防ぐ）。
        // NPC（行商人など）が carve point に乗ると通路を塞ぎうるので、NPC ブロックも考慮して到達判定する。
        const ex = Object.values(m.exits)[0];
        if (ex) {
          const reach = reachable(m, camp.x, camp.y, npcBlockedSet(m));
          // 逆方向＝野営地から exit タイルへ到達できる（連結性は対称）。
          const gate = Object.keys(m.exits)[0]!;
          const gp = findChar(m, gate);
          if (gp) expect(reach.has(`${gp.x},${gp.y}`), `${id}: 野営地から出口'${gate}'へ到達不可`).toBe(true);
        }
      }
    }
  });
